...
import { GET_GALLARY } from "../../../../apolloClient/queries/historicalGallary/historicalGalleryQuery";
import useDeleteGalary from "../../../../apolloClient/mutation/history/delete";
...
const galary = ({ token, currentUser }) => {
  ...
  const { data, loading, error } = useQuery(GET_GALLARY, {
    context: {
      headers: {
  ...
  const { deleteHistoricalSlider } = useDeleteGalary();
  ...
  const emptyRows =
    page > 0
      ? Math.max(
          0,
          (1 + page) * rowsPerPage -
            data?.me?.consumers?.country?.historicalsliders?.edges?.length
        )
      : 0;

  //Show bank data and searches real time
  const tableDatas =
    data?.me?.consumers?.country?.historicalsliders?.edges?.filter((search) => {
    ...
    .map((row, index) => {
      ...
      return (
        ...
        >
          ...
          >
            <Link
              href={`/consumer-dashboard/info/galary/${row.node.id}/`}
              passHref
            >
    ...
    <div>
      ...
      <Box sx={{ width: "100%" }} style={{ paddingRight: "5px" }}>
        ...
        >
          ...
          >
            <Add />
            <Link href="/consumer-dashboard/info/galary/addGallary">
              Add Gallery
            </Link>
          </Typography>
...
export default withConsumerAuth(galary);
