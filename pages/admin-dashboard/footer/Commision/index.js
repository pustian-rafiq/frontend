...
import { GET_TERMS } from "../../../../apolloClient/queries/termsCondition/termscondition";
import useDeleteTerms from "../../../../apolloClient/mutation/termsConditions/deleteterms";
...
const headCells = [
  ...
  {
    id: "Commosition Title",
    numeric: false,
    disablePadding: false,
    label: "commision title list",
  },
...
const CommisionAll = ({ token, currentUser }) => {
...
//delete items from the gql 
  const { deleteTermCondition } = useDeleteTerms();
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - data.termConditions.edges.length)
      : 0;

  //Show bank data and searches real time
  const tableDatas = data.termConditions.edges.filter((search) => {
      ...
      return (
        ...
        >
          ...
          <TableCell className="tableBorder" align="left">
            <div dangerouslySetInnerHTML={{__html:row?.node?.description}}></div>
          ...
          >
            <Link
              href={`/admin-dashboard/media-info/terms/${row.node.id}/`}
              passHref
            >
    ...
    <div>
      ...
      <Box sx={{ width: "100%" }} style={{ paddingRight: "5px" }}>
        ...
        >
          ...
          >
            <Add />
            <Link href="/admin-dashboard/footer/Commision/add">
              Add Commision
            </Link>
          </Typography>
          ...
          <TablePagination
            ...
            component="div"
            count={data.termConditions.edges.length}
...
export default withAdminAuth(CommisionAll);
