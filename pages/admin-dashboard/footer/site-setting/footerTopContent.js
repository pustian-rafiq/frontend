...
import { Badge, Button, Card, CardContent, Divider, FormControlLabel, Grid, IconButton, Modal, TextField, Tooltip, Typography } from '@mui/material';
...
import { GET_FOOTERTOP_CONTENT } from '../../../../apolloClient/queries/adminDashboard/Footer/GetFooterTopContent';
...
// Footer Top Content Component 
const footerTopContent = ({ token }) => {
  ...
  const { data: footerTopContentData, error, loading } = useQuery(GET_FOOTERTOP_CONTENT, {
    context: {
      headers: {
  ...
  useEffect(() => {
    setItems(footerTopContentData?.footerTopContents?.edges);
  }, [footerTopContentData]);
  ...
  const onDragEnd = (result) => {
    ...
    const reorderedItems = reorder(
      items.length > 0 ? items : footerTopContentData?.footerTopContents?.edges,
      result.source.index,
...
export default withAdminAuth(footerTopContent)
