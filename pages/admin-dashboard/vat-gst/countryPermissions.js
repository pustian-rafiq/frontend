...
const CountryPermissions = ({ token, currentUser }) => {
  ...
  const tableDatas = rows.filter((search) => {
    return search.country.toLowerCase().includes(searchText.toLowerCase());
  });
    ...
    .map((row, index) => {
      ...
      return (
        ...
        >
          ...
          >
            <Link
              href={`/admin-dashboard/vat-gst/vat-update/${row.no}/`}
              passHref
            >
              <Typography
                variant="span"
...
export default withAdminAuth(CountryPermissions);
