...
import { GET_SLIDERS } from "../../../../apolloClient/queries/heroSlider/heroSlider";
import useDeleteSlider from "../../../../apolloClient/mutation/slider/deleteSlider";
...
const SliderList = ({ token, currentUser }) => {
  ...
  const { data, loading, error } = useQuery(GET_SLIDERS, {
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - data.sliders.edges.length)
      : 0;

  //Show bank data and searches real time
  const tableDatas = data.sliders.edges.filter((search) => {
    ...
    .map((row, index) => {
      ...
      return (
        ...
        >
          ...
          >
            <Link
              href={`/admin-dashboard/media-info/sliders/${row.node.id}/`}
              passHref
            >
      ...
      <Box sx={{ width: "100%" }} style={{ paddingRight: "5px" }}>
        ...
        >
          ...
          <TablePagination
            ...
            component="div"
            count={data.sliders.edges.length}
...
export default withAdminAuth(SliderList);
