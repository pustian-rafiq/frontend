...
import useDeleteSubSubCategory from '../../../../../apolloClient/mutation/inventory/DeleteSubSubCategory';
...
const ShowCategory = ({ subSubCategories, getSessionCookie }) => {
    ...
    const subSubCategoryDeleteHandler = (id) => {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    subSubCategoryDeleteMutation({
                        ...
                        refetchQueries: [{ query: GET_SUB_SUBCATEGORIES }],
                        onCompleted: () => {
                    ...
                    reset();
                    swal("Hey! Sub Sub-Category has been deleted!", {
                        icon: "success",
                    });
                } else {
                    swal("Sub Sub-Category is safe!");
                }
            });
    ...
    //Show sub sub-category data and searches real time
    const tableDatas = subSubCategories.filter((search) => {
        return (
            search?.node?.name.toLowerCase().includes(searchText.toLowerCase()) ||
            search?.node?.subCategory?.name.toLowerCase().includes(searchText.toLowerCase())
        );
    });
        ...
        .map((row, index) => {
            ...
            return (
                <TableRow
                    ...
                    className="rowHover"
                >
                    <TableCell className="tableBorder" align="left">{(page * rowsPerPage) + (++index)}</TableCell>
                    <TableCell className="tableBorder" align="left">{row?.node?.name}</TableCell>
                    <TableCell className="tableBorder" align="left">{row?.node?.subCategory?.name}</TableCell>
                    <TableCell className="tableBorder" align="left">
                        <img src={row?.node?.image} width={50} height={50} />
                    </TableCell>
                    <TableCell className="tableBorder" align="left" sx={{ width: '130px' }}>
                        <Link href={`/admin-dashboard/inventory/edit-subsubcategory/${row?.node?.id}/`} passHref>
                            <Tooltip title="Update Sub Sub-Category">
                        ...
                        <Tooltip title="Delete Sub-Category">
                            <IconButton
                                onClick={() => subSubCategoryDeleteHandler(row.node.id)}
                                color="warning"
                            >
