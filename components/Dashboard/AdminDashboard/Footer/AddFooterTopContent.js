...
import { GET_FOOTERTOP_CONTENTENT } from '../../../../apolloClient/queries/adminDashboard/Footer/GetFooterTopContent';
...
const AddFooterTopContent = ({token}) => {

    const { footerTopContentMutationHandler, loading, error, data } =
 ...
 const submitHandler = (data) => {
  ...
  footerTopContentMutationHandler({
      variables: {
        name: data.name,
        link: data.link,
      },
      refetchQueries: [{ query: GET_FOOTERTOP_CONTENTENT }],
     // refetchQueries: 
      context: {
        headers: {
      ...
      onError: (err) => {
        ...
        toast.warn("Footer content not added!", {
          position: "top-center",
          autoClose: 3000,
        });
      },
...
export default AddFooterTopContent