...
import useMunicipalityMutation from "../../../../apolloClient/mutation/Settings/Address/AddMunicipality";
...
const AddMunicipality = ({
  ...
  const { municipalityMutationHandler, error, loading, data } =
    useMunicipalityMutation();
  ...
  const municipalitySubmitHandler = (data) => {
    if (loading) {
      return <div>Loading......</div>;
    }
    municipalityMutationHandler({
      variables: {
        name: data.name,
        policeStationId: policeStationId,
      },
      refetchQueries: [GET_MUNICIPALITIES],
      ...
      onError: (err) => {
        ...
        toast.warn("Municipality not added!", {
          position: "top-center",
          autoClose: 3000,
        });
      },
      ...
      >
        <Box sx={{ ...style, minWidth: 300 }}>
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", mt: 2 }}>
            <form onSubmit={handleSubmit(municipalitySubmitHandler)}>
                ...
                <Grid item xs={9} md={10}>
                  ...
                  <Typography sx={{ color: "#E75C33" }}>
                    {errors.name &&
                      ...
                      "You must have a municipality name"}
                  </Typography>
                </Grid>
